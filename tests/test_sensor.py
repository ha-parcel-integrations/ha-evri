"""Tests for Evri sensor property logic."""

from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.evri.const import ParcelStatus
from custom_components.evri.sensor import (
    EvriAwaitingPickupSensor,
    EvriDeliveredParcelsSensor,
    EvriIncomingParcelsSensor,
    EvriLastUpdateSensor,
    EvriNextDeliverySensor,
    EvriOutgoingDeliveredParcelsSensor,
    EvriOutgoingParcelsSensor,
    EvriParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = EvriIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = EvriParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = EvriParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = EvriNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = EvriNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = EvriDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = EvriLastUpdateSensor(coordinator, _entry())
    assert sensor.native_value == moment


def test_pickup_and_outgoing_summaries():
    coordinator = _coordinator([_parcel("P", ParcelStatus.AT_PICKUP_POINT, True)])
    assert EvriAwaitingPickupSensor(coordinator, _entry()).native_value == 1
    coordinator.outgoing = [_parcel("O")]
    coordinator.delivered_outgoing = [_parcel("D", ParcelStatus.DELIVERED)]
    outgoing = EvriOutgoingParcelsSensor(coordinator, _entry())
    delivered = EvriOutgoingDeliveredParcelsSensor(coordinator, _entry())
    assert outgoing.native_value == 1
    assert outgoing.extra_state_attributes["parcels"][0]["barcode"] == "O"
