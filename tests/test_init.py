"""Tests for Evri setup and unload."""
...
from custom_components.evri.api import EvriApiError
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.evri.api.EvriApiClient.async_get_parcel",
    ...
    incoming = hass.states.get("sensor.evri_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.evri.api.EvriApiClient.async_get_parcel",
        new=AsyncMock(side_effect=EvriApiError("Evri unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.evri.api.EvriApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
...
async def test_options_update_applies_live_without_reload(hass):
    ...
    with patch("custom_components.evri.api.EvriApiClient.async_get_parcel", new=mock):
        ...
        mock.side_effect = lambda code, postcode: _sample(code)
...
async def test_services_remain_until_final_hub_unloads(hass):
    first = MockConfigEntry(
        domain=DOMAIN, unique_id="SW1A1AA", options={CONF_PARCELS: []}
    )
    second = MockConfigEntry(
        domain=DOMAIN, unique_id="EC1A1BB", options={CONF_PARCELS: []}
    )
