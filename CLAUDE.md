...
Home Assistant custom integration for **Evri** parcel tracking.
...
private `carrier-research/evri/api/` and are **never** copied here.
...
- **If this carrier can reach `ParcelStatus.AT_PICKUP_POINT` from a real raw
  ...
  `ha-inpost` are reference implementations. Evri reaches this status through
  its pickup-point stage codes and exposes the sensor.

## Carrier-specific notes

- Evri is account-less and supports multiple hubs, keyed by an immutable,
  normalized UK delivery postcode. Options apply live; services require
  `postal_code` only when several hubs make the target ambiguous.
- Incoming versus outgoing is declared per tracked code. Re-filing a code in
  the other list moves it, and per-parcel sensors search both active buckets.
- The carrier's public website credential is shared transport state, not user
  authentication. Its rejection fails the whole poll as an integration outage
  and never opens a Home Assistant reauthentication flow.
- Opaque parcel identifiers are cached only in memory. Last-good raw responses
  keep entities stable through transient errors, delivered codes are skipped,
  and credential failure stops the remaining requests for that hub.
- `raw` is a deliberate privacy allowlist. Recipient and address data is never
  published. ETA, receiver, weight, dimensions and pickup-point name remain
  `None` until reviewed redacted fixtures establish safe semantics.
- `awaiting_pickup` is included for parcels ready at a pickup point.
...
Home Assistant (`EvriAuthError` from any per-parcel fetch raises
...
refresh. `evri/coordinator.py` is the canonical implementation
...
```
python -m pytest tests/ --cov=custom_components.evri
```
