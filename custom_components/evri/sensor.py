"""Sensor platform for the Evri parcel tracker integration."""
...
def _active_parcels(coordinator: EvriCoordinator) -> list[dict]:
    return list(coordinator.data or []) + list(coordinator.outgoing or [])


def _active_barcodes(coordinator: EvriCoordinator) -> set[str]:
    return {p.get("barcode", "") for p in _active_parcels(coordinator)}


async def async_setup_entry(
    hass: HomeAssistant,
    entry: EvriConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Evri sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        EvriIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        EvriAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in _active_parcels(coordinator):
        entities.append(EvriParcelSensor(coordinator, entry, parcel.get("barcode", "")))
    entities.append(EvriNextDeliverySensor(coordinator, entry))
    entities.append(EvriDeliveredParcelsSensor(coordinator, entry))
    entities.append(EvriOutgoingParcelsSensor(coordinator, entry))
    entities.append(EvriOutgoingDeliveredParcelsSensor(coordinator, entry))
    entities.append(EvriLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class EvriIncomingParcelsSensor(CoordinatorEntity[EvriCoordinator], SensorEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                EvriParcelSensor(self.coordinator, self._entry, barcode)
...
class EvriAwaitingPickupSensor(CoordinatorEntity[EvriCoordinator], SensorEntity):
    """Incoming parcels ready to collect at a pickup point."""
    ...
    def __init__(self, coordinator: EvriCoordinator, entry: ConfigEntry) -> None:
        """Initialize the pickup summary."""
        super().__init__(coordinator)
    ...
    @property
    def native_value(self) -> int:
        """Return the number of incoming parcels ready for pickup."""
        return len(self._parcels())

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return incoming parcels ready for pickup."""
        return {"parcels": self._parcels()}


class EvriParcelSensor(CoordinatorEntity[EvriCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Evri parcel."""
    ...
    def __init__(
        self, coordinator: EvriCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class EvriNextDeliverySensor(CoordinatorEntity[EvriCoordinator], SensorEntity):
...
class EvriDeliveredParcelsSensor(CoordinatorEntity[EvriCoordinator], SensorEntity):
    """Recently delivered tracked Evri parcels."""
...
class EvriOutgoingParcelsSensor(CoordinatorEntity[EvriCoordinator], SensorEntity):
    ...
    def __init__(self, coordinator: EvriCoordinator, entry: ConfigEntry) -> None:
        """Initialize the outgoing summary."""
        super().__init__(coordinator)
    ...
    @property
    def native_value(self) -> int:
        """Return the active outgoing parcel count."""
...
class EvriOutgoingDeliveredParcelsSensor(
    CoordinatorEntity[EvriCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: EvriCoordinator, entry: ConfigEntry) -> None:
        """Initialize the delivered outgoing summary."""
        super().__init__(coordinator)
    ...
    @property
    def native_value(self) -> int:
        """Return the retained delivered outgoing count."""
        return len(self.coordinator.delivered_outgoing or [])

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return retained delivered outgoing parcels."""
        return {"parcels": self.coordinator.delivered_outgoing or []}


class EvriLastUpdateSensor(CoordinatorEntity[EvriCoordinator], SensorEntity):
    """Diagnostic sensor reporting when Evri was last polled successfully."""
