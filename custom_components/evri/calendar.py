"""Calendar platform for the Evri parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Evri deliveries calendar from a config entry."""
    async_add_entities([EvriDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class EvriDeliveriesCalendar(CoordinatorEntity[EvriCoordinator], CalendarEntity):
    """A read-only calendar of expected Evri deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Evri parcel")
            description_parts = [
