"""Coordinator for the Evri parcel tracker integration.
...
from .api import EvriApiClient, EvriApiError, EvriCredentialError
from .const import (
...
class EvriCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    def _tracked(self) -> dict[str, str]:
        """Return configured codes mapped to their declared direction."""
        return {
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        postcode = self.config_entry.options.get(CONF_POSTAL_CODE, "")
        ...
        for code in codes_to_fetch:
            try:
                results.append(await self._client.async_get_parcel(code, postcode))
            except EvriCredentialError as err:
                raise UpdateFailed(
                    "Evri rejected its public tracking credential"
                ) from err
            except (EvriApiError, aiohttp.ClientError) as err:
                results.append(err)
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(result, (EvriApiError, aiohttp.ClientError)):
                    raise result
                errors += 1
                if isinstance(result, EvriApiError) and result.status_code == 429:
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("Evri fetch failed for a tracked parcel: %s", result)
            ...
            # its key.
            result.setdefault("_configuredTrackingCode", code)
        ...
        if saw_429:
            ...
            raise UpdateFailed("Evri rate-limited (429)", retry_after=retry_after)
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed(
                "Evri's public tracking endpoint is unavailable for all tracked "
                "parcels"
            )
    ...
    def _fire_outgoing_change_events(self, parcels: list[dict]) -> None:
        """Fire the outgoing status and delivered event pair."""
