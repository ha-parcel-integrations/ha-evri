"""Evri parcel tracker custom component for Home Assistant."""
...
@dataclass
class EvriData:
    """Runtime data attached to the Evri config entry."""

    client: EvriApiClient
    coordinator: EvriCoordinator


type EvriConfigEntry = ConfigEntry[EvriData]


async def async_setup_entry(hass: HomeAssistant, entry: EvriConfigEntry) -> bool:
    """Set up Evri from a config entry."""
    # No auth: Evri tracking is public, so the HA-managed session is fine.
    client = EvriApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = EvriData(client=client, coordinator=coordinator)
...
async def _async_options_updated(hass: HomeAssistant, entry: EvriConfigEntry) -> None:
    """Apply changed options by refreshing the coordinator."""
    await entry.runtime_data.coordinator.async_request_refresh()


async def async_unload_entry(hass: HomeAssistant, entry: EvriConfigEntry) -> bool:
    """Unload the Evri config entry."""
