# Automatic parcel tracking from e-mail (IMAP → Evri)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Evri tracking code, and register it with `evri.track_parcel` — fully automatic, no extra custom component required.

Evri is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `evri.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ evri.track_parcel
                                     │
...
- This integration, with the `evri.track_parcel` action available (field `tracking_code`).
...
The regex matches Evri's currently advertised 16-character alphanumeric shape:

```
(?<![0-9A-Z])[0-9A-Z]{16}(?![0-9A-Z])
```

The integration itself accepts every non-empty normalized code so a future Evri format is not rejected. The e-mail example stays narrower to avoid mistaking every order number for a parcel code; use the AI fallback or adjust the regex if your Evri messages use another shape.
...
1. **Jinja eats backslashes in string literals.** A template using `\b` can silently gain a **backspace character** because it is a string escape. That's why this pattern uses backslash-free lookarounds and character classes.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@evrishipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: H00AAA0000000001","initial":true,"folder":"INBOX","username":"test"}'
```

Then `evri.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
